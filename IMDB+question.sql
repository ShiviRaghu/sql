...
SELECT production_company, COUNT(*) as total FROM movie group by production_company having count(*)<1;
...
select year ,count(distinct id) from movie group by year;
 select extract(month from date_published) as month_number, count(distinct id) from movie group by 
 month_number order by count(distinct id) desc ;
...
from movie where upper(country) in ('USA','INDIA') AND YEAR = 2019
group by country;
...
select  genre ,count(distinct movie_id) from genre group by genre order by count(distinct movie_id) desc;
...
select count(movie_id) from genre where movie_id in
...
/* Now you know, movies of genre 'Drama' (produced highest in number in 2019) 
has the average duration of 106.77 mins.
Lets find where the movies of genre 'thriller' on the basis of number of movies.*/

-- Q9.What is the rank of the ‘thriller’
